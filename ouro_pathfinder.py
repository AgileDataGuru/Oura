...
query = "select * from (select i.ticker, i.RSIVOTE + i.MACDVOTE + i.AROONVOTE + i.CCIVOTE as Vote from daily_indicators i where i.tradedate = '2020-04-29') x where x.Vote > 0"
try:
    stocklist = list(indicators.query_items(
        query=query,
...
stocks = pd.DataFrame(stocklist).sort_values(by=['Vote'], ascending=False)
print (stocks)
